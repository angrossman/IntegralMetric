...
@register_cell_magic
def pylint(line, cell):
    # Сохранение содержимого ячейки в файл
    with open("temp_script.py", "w") as file:
        file.write(cell)
    
    # Запуск Pylint и получение результатов
    result = subprocess.run(['pylint', 'temp_script.py'], stdout=subprocess.PIPE, text=True)
    display(Markdown(f'```\n{result.stdout}\n```'))

# Использование магической команды %%pylint для анализа кода
...
"""
Создаем классы под нашу BPMN схему рабочего процесса.
"""
...
class RawData:
    ...
    #Сырые данные
    """
    def __init__(self, raw_data_id, data_source,
                 data_content, collection_date):
...
class CleanedData:
    ...
    Очищенные данные
    """
    def __init__(self, cleaned_data_id, raw_data_id,
                 created_by, cleaned_content, cleaning_date):
...
class Task:
    ...
    Задача
    """
    def __init__(self, task_id, assigned_to, task_description,
                 status, due_date, completion_time, reworks):
...
class Report:
    ...
    Отчет
    """
    def __init__(self, report_id, cleaned_data_id, user_id,
                 task_id, approved_by, content, creation_date):
...
class Recommendation:
    """
    Рекомендации к отчету
    """
    def __init__(self, recommendation_id, user_id, approved_by,
                 report_id, task_id, content, creation_date):
...
class Strategy:
    ...
    Стратегия
    """
    def __init__(self, strategy_id, report_id, recommendation_id,
                 task_id, user_id, content, creation_date):
...
"""
В этом разделе наполним наши таблицы сгенерированными данными
"""

def generate_random_data(num_entries):
    """
    Прописываем условия генерирования данных
    """
    raw_data_list = []
    cleaned_data_list = []
    task_list = []
    ...
    for i in range(num_entries):
        user = User(
            user_id=i,
            name=fake.name(),
            role=random.choice(['Руководитель', 'Стратег', 'Аналитик данных',
                                'Маркетинговый аналитик', 'Помощник', 'SMM-спец', 'SEO-спец']),
            email=fake.email()
        )
...
"""
В этом разделе выведем сгенерированные данные на проверку
"""
...
#%%pylint
"""
В этом разделе считаем интегральную метрику
"""

def integral_metric(tasks, max_allowed_time):
    """
    Функция по расчету интегральной метрики
    """
    alpha = 0.5
    beta = 0.5
